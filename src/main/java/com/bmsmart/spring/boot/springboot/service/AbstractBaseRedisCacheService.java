 ...
 * @return
 */
public abstract class AbstractBaseRedisCacheService {
    ...
    public ClusterInfo getRedisClusterInfo() {
        RedisClusterConnection clusterConnection = this.getRedisTemplate().getConnectionFactory().getClusterConnection();
    ...
    public long getPTtl(String key) {
        return this.getRedisTemplate().getExpire(key, TimeUnit.MILLISECONDS);
    }
    ...
    public void setExpire(String key) {
        if (expireTime > 0) {
            this.getRedisTemplate().expire(key, this.expireTime, TimeUnit.SECONDS);
        }
    }
    ...
    public <T> ValueOperations<String, T> setCacheObject(String key, T value) {
        ...
        ValueOperations<String, T> operation = this.getRedisTemplate().opsForValue();
    ...
    public <T> ListOperations<String, T> setCacheList(String key, List<T> dataList) {
        ListOperations listOperation = this.getRedisTemplate().opsForList();
        if (null != dataList) {
    ...
    public <T> List<T> getCacheList(String key) {
        ...
        ListOperations<String, T> listOperation = this.getRedisTemplate().opsForList();
    ...
    public <T> BoundSetOperations<String, T> setCacheSet(String key, Set<T> dataSet) {
        this.delete(key);
        BoundSetOperations<String, T> setOperation = this.getRedisTemplate().boundSetOps(key);
...
    setOperation.add(t);*/
    ...
    public Set<Object> getCacheSet(String key) {
        ...
        BoundSetOperations<String, Object> operation = this.getRedisTemplate().boundSetOps(key);
    ...
    public <T> BoundZSetOperations<String, T> setCacheZSet(String key, Set<T> dataSet) {
        this.delete(key);
        BoundZSetOperations<String, T> setOperation = this.getRedisTemplate().boundZSetOps(key);
        T[] t = (T[]) dataSet.toArray();
    ...
    public Set<Object> getCacheZSet(String key) {
        BoundZSetOperations<String, Object> operation = this.getRedisTemplate().boundZSetOps(key);
        Long size = operation.size();
    ...
    public <T> HashOperations<String, String, T> setCacheMap(String key, Map<String, T> dataMap) {
        ...
        HashOperations hashOperations = this.getRedisTemplate().opsForHash();
        if (null != dataMap) {
    ...
    public <T> Map<String, T> getCacheMap(String key/*,HashOperations<String,String,T> hashOperation*/) {
        Map<String, T> map = this.getRedisTemplate().opsForHash().entries(key);
    ...
    public <T> Map<Integer, T> getCacheIntegerMap(String key/*,HashOperations<String,String,T> hashOperation*/) {
        Map<Integer, T> map = this.getRedisTemplate().opsForHash().entries(key);
        /*Map<String, T> map = hashOperation.entries(key);*/
        return map;
    }
